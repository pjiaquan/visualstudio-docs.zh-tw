...
#### 參數  
 `pszCode`  
 \[in， size\_is \(`cch`\)\]。scriptlet 文字。  不需要是以 null 結束之字串。  
  
 `cch`  
 \[out\] 用來 `pszCode` 和 `pattr` 參數的大小。  
  
 `pszDelimiter`  
 \[in\] 位址結束 scriptlet 符號。  當 `pszCode` 從文字資料流剖析時，主應用程式通常會使用分隔符號 \(例如兩個單引號\)， scriptlet 偵測到的結尾。  如果某個符號不是用來識別 scriptlet 的結尾，請將此參數設定為 NULL。  
  
 `dwFlags`  
 \[in\] 與 scriptlet 文字屬性的旗標。  可以是下列值的組合。  
...
|GETATTRFLAG\_HUMANTEXT|0x8000|識別字串內容和註解。SOURCETEXT\_ATTR\_HUMANTEXT 具有屬性的文字。|  
 ...
 `pattr`  
 \[in, out, size\_is\(`cch`\)\] scriptlet 程式碼的色彩資訊。  
