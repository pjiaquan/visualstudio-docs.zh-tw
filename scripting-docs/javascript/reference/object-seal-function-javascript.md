---
title: "Object.seal 函式 (JavaScript) | Microsoft Docs"
ms.custom: ""
ms.date: "01/18/2017"
...
---
# Object.seal 函式 (JavaScript)
避免修改現有屬性 \(Property\) 的屬性 \(Attribute\)，並避免加入新的屬性 \(Property\)。  
...
## 備註  
 `Object.seal` 函式會執行下列兩項工作：  
  
-   讓此物件無法擴充，好讓新的屬性無法加入其中。  
  
-   針對物件的所有屬性 \(Property\) 將 `configurable` 屬性 \(Attribute\) 設定為 `false`。  
  
 當 `configurable` 屬性 \(Attribute\) 為 `false` 時，表示屬性 \(Property\) 的屬性 \(Attribute\) 無法變更，也無法刪除此屬性 \(Property\)。  當 `configurable` 為 `false` 且 `writable` 為 `true` 時，可以變更 `value` 和 `writable` 屬性。  
