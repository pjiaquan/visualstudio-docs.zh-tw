...
介面中的事件不能有 add 或 remove 存取子  
 ...
 您不能在 [interface](/dotnet/csharp/language-reference/keywords/interface) 中定義事件的存取子函式。 如需詳細資訊，請參閱[事件](/dotnet/csharp/programming-guide/events/index)與[介面](/dotnet/csharp/programming-guide/interfaces/index)。  
