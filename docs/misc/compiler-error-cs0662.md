...
'method' 不能在 ref 參數上只指定 Out 屬性。 請同時使用 In 與 Out 屬性，或都不使用。  
 ...
 介面方法所包含的參數，使用了僅指定 [Out](frlrfSystemRuntimeInteropServicesOutAttributeClassTopic) 屬性的 [ref](/dotnet/csharp/language-reference/keywords/ref)。 使用 **Out** 屬性的 `ref` 參數也必須使用 [In](frlrfSystemRuntimeInteropServicesInAttributeClassTopic) 屬性。  
