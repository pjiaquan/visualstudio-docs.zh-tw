...
加入的模組必須以 CLSCompliant 屬性標記，才能與這個組件相符  
 ...
 已使用 [\/addmodule](/dotnet/csharp/language-reference/compiler-options/addmodule-compiler-option)，將使用 [\/target:module](../Topic/-target:module%20\(C%23%20Compiler%20Options\).md) 編譯器選項所編譯的模組加入編譯。 不過，模組與 Common Language Specification \(CLS\) 的符合性，與目前編譯的 CLS 狀態不一致。  
