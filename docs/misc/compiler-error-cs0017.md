...
程式 'output file name' 有一個以上的已定義進入點。 請以 \/main 編譯以指定包含進入點的類型。  
 ...
 程式只能有一個 [Main](/dotnet/csharp/programming-guide/main-and-command-args/main-and-command-line-arguments) 方法。  
  
 若要解決這個錯誤，您可以刪除程式碼中的所有 Main 方法，只保留一個，或使用 [\/main](/dotnet/csharp/language-reference/compiler-options/main-compiler-option) 編譯器選項，指定要使用的 Main 方法。  
