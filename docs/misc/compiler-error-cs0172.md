...
無法判斷條件運算式的類型，因為 'type1' 和 'type2' 會互相隱含轉換。  
 ...
 在條件陳述式中，您必須可以轉換 `:` 運算子任一側的類型。 此外，不能有相互轉換常式；您只需要一個轉換。 如需詳細資訊，請參閱[轉換運算子](/dotnet/csharp/programming-guide/statements-expressions-operators/conversion-operators)。  
