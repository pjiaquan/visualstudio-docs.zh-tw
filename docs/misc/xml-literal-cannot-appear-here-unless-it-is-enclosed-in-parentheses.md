...
運算式中使用的 XML 常值宣告，其位置對 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 編譯器來說模稜兩可。 也就是說，[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 編譯器無法判斷小於字元 \(\<\) 要當做比較運算子還是 XML 常值的開頭。 下列程式碼提供一個範例：  
