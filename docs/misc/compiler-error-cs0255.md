...
在 catch 或 finally 區塊中不可使用 stackalloc  
 ...
 在 [catch](/dotnet/csharp/language-reference/keywords/try-catch) 或 [finally](/dotnet/csharp/language-reference/keywords/try-catch-finally) 區塊中不可使用 [stackalloc](/dotnet/csharp/language-reference/keywords/stackalloc) 關鍵字。 如需詳細資訊，請參閱[例外狀況和例外狀況處理](/dotnet/csharp/programming-guide/exceptions/exceptions-and-exception-handling)。  
