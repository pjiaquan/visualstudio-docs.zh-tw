...
預設屬性中的 [Get Statement](/dotnet/visual-basic/language-reference/statements/get-statement) 或 [Set Statement](/dotnet/visual-basic/language-reference/statements/set-statement) 包含 `Private` 關鍵字。  
...
-   請從 `Get` 或 `Set` 陳述式中移除 `Private` 關鍵字，或從 [Property Statement](/dotnet/visual-basic/language-reference/statements/property-statement) 中移除 `Default`。  
