...
如果定義類型參數，但其條件約束清單中沒有 [Class \(Visual Basic\)](http://msdn.microsoft.com/zh-tw/0777c6e6-46bc-451b-ad70-57b49d4ef4f7) 關鍵字或特定類別名稱，則類型參數是作為 [IsNot Operator](/dotnet/visual-basic/language-reference/operators/isnot-operator) 的運算元。  
  
 `IsNot` 會比較兩個參考類型，以判斷它們是否指向記憶體中的不同物件執行個體。 除非另一個運算元是 [Nothing](/dotnet/visual-basic/language-reference/nothing)，否則它無法使用不是參考類型的運算元。  
