...
運算子 '\<運算子符號\>' 在所有程式碼路徑上皆不會傳回值。 使用結果時，Null 參考例外狀況可能在執行階段時發生。  
 ...
 您只能藉由將運算子程序包含在 [Return Statement](/dotnet/visual-basic/language-reference/statements/return-statement)，以從運算子程序傳回值。  
  
 如果控制權傳遞給 `End Operator` 陳述式，運算子程序會傳回屬性資料類型的預設值。 如需詳細資訊，請參閱 [Function Statement](/dotnet/visual-basic/language-reference/statements/function-statement)中的＜行為＞。  
