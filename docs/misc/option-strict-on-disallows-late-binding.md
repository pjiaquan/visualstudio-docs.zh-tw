...
[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 允許將任何資料類型隱含地轉換成任何其他資料類型。 不過，如果將某種資料類型的值轉換成精確度較低或容量較小的資料類型，則資料可能會遺失。`Option Strict On` 可確保這些類型之轉換的編譯時期通知，因而予以避免。 您不能使用具有晚期繫結的 `Option Strict On`。  
