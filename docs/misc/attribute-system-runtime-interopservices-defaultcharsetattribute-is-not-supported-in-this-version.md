...
<xref:System.Runtime.InteropServices.DefaultCharSetAttribute?displayProperty=fullName> 屬性可讓您指定要用於封送處理字串的字元集。 它的值會採用 <xref:System.Runtime.InteropServices.CharSet?displayProperty=fullName> 列舉的成員。  
 ...
 目前的 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 版本不支援這個屬性。 可能會在未來版本中進行支援。  
  
 **錯誤 ID︰**BC32510  
  
### 更正這個錯誤  
  
-   使用每個 [Declare Statement](/dotnet/visual-basic/language-reference/statements/declare-statement) 來指定所宣告外部程序的字元集。 下列範例將說明這點。  
