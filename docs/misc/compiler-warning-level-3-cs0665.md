...
條件運算式中的指派一直是常數; 這表示您要使用 \=\= 代替 \= ?  
 ...
 條件運算式已使用 [\= 運算子](/dotnet/csharp/language-reference/operators/assignment-operator)，而非 [\=\= 運算子](/dotnet/csharp/language-reference/operators/equality-comparison-operator)。  
