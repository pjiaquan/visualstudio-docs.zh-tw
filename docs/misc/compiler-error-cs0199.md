...
不能傳遞靜態唯讀欄位 'name' 的欄位給 ref 或 out \(除非在靜態建構函式中\)  
 ...
 [readonly](/dotnet/csharp/language-reference/keywords/readonly) 變數的 [static](/dotnet/csharp/language-reference/keywords/static) 使用情形必須與建構函式相同，而在此建構函式中，您想要將它傳遞為 [ref](/dotnet/csharp/language-reference/keywords/ref) 或 [out](/dotnet/csharp/language-reference/keywords/out) 參數。 如需詳細資訊，請參閱[傳遞參數](/dotnet/csharp/programming-guide/classes-and-structs/passing-parameters)。  
