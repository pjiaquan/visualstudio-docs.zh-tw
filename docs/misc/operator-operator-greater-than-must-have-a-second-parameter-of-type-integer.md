...
位元移位運算子和不是 `Integer` 的第二個類型參數一起宣告。  
 ...
 當您在運算式中使用右移 \(`>>`\) 或左移 \(`<<`\) 運算子時，您可以在第二個運算元中指定移位量。 因為這個運算元的緣故，[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 允許您提供任何可擴展為 `Integer` 的資料類型。 不過，第二個運算元的定義嚴定為 `Integer`。 如果用類別或結構上的位元移位運算子定義該類別或結構，第二個運算元的定義必須指定 `Integer`。  
