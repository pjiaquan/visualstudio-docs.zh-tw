...
## 範例  
 下列程式碼會產生 CS1955，因為程式碼嘗試使用方法呼叫運算子 [\(\)](/dotnet/csharp/language-reference/operators/invocation-operator) 來叫用欄位和屬性。 您不能呼叫欄位或屬性，但可以使用成員存取運算子 \([.](/dotnet/csharp/language-reference/operators/member-access-operator)\) 來存取它所儲存的值。  
