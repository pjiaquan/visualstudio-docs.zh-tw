...
1.  建立工具視窗窗格、框架以及實作它們的 VSPackage。 如需詳細資訊，請參閱[加入工具視窗](../extensibility/adding-a-tool-window.md)。  
...
2.  將 <xref:Microsoft.VisualStudio.Shell.ProvideToolWindowAttribute> 加入提供它的 VSPackage。  
    ...
    ```  
     ...
     這會註冊以停駐形式開啟到**方案總管**的工具視窗 PersistedWindowPane。 如需詳細資訊，請參閱[註冊工具視窗](../extensibility/registering-a-tool-window.md)。  
