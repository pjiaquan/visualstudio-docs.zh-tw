...
衍生類別不會宣告建構函式，而且 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 無法產生建構函式，因為它無法判定要呼叫的基底類別建構函式。  
  
 當衍生類別未宣告建構函式時，[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 會嘗試產生呼叫 `MyBase.New()` 的隱含無參數建構函式。 如果可以不使用引數呼叫的基底類別中沒有可存取的建構函式，或是有多個可存取的建構函式，[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 就無法產生隱含的建構函式。  
