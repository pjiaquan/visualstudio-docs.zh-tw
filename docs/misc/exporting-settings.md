...
---
# 匯出設定
[!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]整合式的開發環境 \(IDE\) 會使用類別來實作<xref:Microsoft.VisualStudio.Shell.IProfileManager>介面和類別，已經註冊為支援特定的 VSPackage 實作儲存 VSPackage 狀態。  
...
1.  宣告實作的類別[!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]設定。  
...
3.  請確定實作設定的類別也會持續存在磁碟的狀態。  
    ...
    -   除了明確的書面且根本不需要的資料<xref:Microsoft.VisualStudio.Shell.Interop.IVsUserSettings.ExportSettings%2A>方法實作中，設定 API 也會儲存[!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]版本資訊。  因此，儲存的設定可以比較這些設定匯入作業期間所產生的 ide 版本。  
...
4.  暫存器的實作類別做為對 VSPackage 提供支援的設定。  
    ...
    ```  
     ...
     在此情況下，屬性會通知 IDE 的`MyPackageProfileManager`類別會提供要設定實作`MyPackage`類別。  自訂設定在登錄中建立點下 HKLM\\Software\\Microsoft\\VisualStudio\\*版本*\\UserSettings\\ CoreUI\_MyPackage，其中*版本*版本的[!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]，比方說，10.0。  
  
     如需詳細資訊，請參閱 [使用者設定的支援](../extensibility/internals/support-for-user-settings.md)和 <xref:Microsoft.VisualStudio.Shell.ProvideProfileAttribute>。  
