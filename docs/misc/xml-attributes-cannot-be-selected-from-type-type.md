...
類型不是 <xref:System.Xml.Linq.XElement> 或 `IEnumerable(Of XElement)` 的物件已參考 XML 屬性。 如需詳細資訊，請參閱[XML Attribute Axis Property](/dotnet/visual-basic/language-reference/xml-axis/xml-attribute-axis-property)。  
