...
無法對靜態唯讀欄位 'name' 的欄位進行指派 \(除非在靜態建構函式或變數初始設定式\)  
 ...
 [readonly](/dotnet/csharp/language-reference/keywords/readonly) 變數的[靜態](/dotnet/csharp/language-reference/keywords/static)使用情形必須與您想要在其中初始化它的建構函式相同。 如需詳細資訊，請參閱[靜態建構函式](/dotnet/csharp/programming-guide/classes-and-structs/static-constructors)。  
