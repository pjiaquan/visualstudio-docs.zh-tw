...
-   [Static](/dotnet/visual-basic/language-reference/modifiers/static)  
 ...
 [Module Statement](/dotnet/visual-basic/language-reference/statements/module-statement) 中唯一支援的關鍵字是 [Public](/dotnet/visual-basic/language-reference/modifiers/public) 和 [Friend](/dotnet/visual-basic/language-reference/modifiers/friend)。  
  
 此外，您不能在模組的陳述式區塊中使用 [Implements](/dotnet/visual-basic/language-reference/statements/implements-clause) 陳述式或 [Inherits Statement](/dotnet/visual-basic/language-reference/statements/inherits-statement)。  
  
 根據預設，這個訊息是一個警告。 如需如何隱藏警告或將警告視為錯誤的詳細資訊，請參閱[在 Visual Basic 中設定警告](../ide/configuring-warnings-in-visual-basic.md)。  
