...
3.  請將設定資料寫入檔案。  
    ...
    2.  VSPackage 必須使用提供的 <xref:Microsoft.VisualStudio.Shell.Interop.IVsSettingsWriter> 介面，將資料儲存至設定檔案。  
        ...
        > [!NOTE]
        >  除了明確寫入的資料以及 <xref:Microsoft.VisualStudio.Shell.Interop.IVsUserSettings.ExportSettings%2A> 實作的透通資料以外，設定 API 也會儲存 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 版本資訊。 儲存的設定可以針對設定匯入期間產生它們的 IDE 版本進行比較。  
