...
運算子 'operator' 用在類型 'type' 的運算元上時其意義會模稜兩可  
 ...
 編譯器有多個可用的轉換，而且不知道套用運算子之前要選擇哪個轉換。 如需詳細資訊，請參閱 [樣板化的使用者定義轉換](../misc/templated-user-defined-conversions.md) 與 [轉換運算子](/dotnet/csharp/programming-guide/statements-expressions-operators/conversion-operators)。  
