...
類型 \('type'\) 必須包含運算子 true 和運算子 false 的宣告  
 ...
 如果您定義了一個使用者定義類型的運算子，然後嘗試使用該運算子作為最少運算運算子，則使用者定義運算子必須已定義運算子 true 和運算子 false。 如需最少運算運算子的詳細資訊，請參閱 [&& 運算子](/dotnet/csharp/language-reference/operators/conditional-and-operator)和 [&#124;&#124; 運算子](../Topic/%7C%7C%20Operator%20\(C%23%20Reference\).md)。  
...
## 請參閱  
 [轉換運算子](/dotnet/csharp/programming-guide/statements-expressions-operators/conversion-operators)