...
![顯示函式簽章的 IntelliSense](~/rtvs/media/intellisense-function-signature.png) 

鍵入函式或其他陳述式時，IntelliSense 會提供自動完成功能表，依您已輸入的內容 (區分大小寫) 來篩選︰

![IntelliSense 自動完成功能表](~/rtvs/media/intellisense-auto-complete-menu.png)

按 Tab 鍵 (或 Enter 鍵、空格鍵，視選項的設定方式而定) 將會插入下拉式清單中選取的項目。 您可以使用方向鍵來變更選取範圍。 

IntelliSense 也提供 R 物件成員的建議︰
 
![物件成員的 IntelliSense 建議](~/rtvs/media/intellisense-auto-complete-r-objects.png)
 
按 Esc 鍵時會一併關閉功能表。 您可以用 Ctrl + 空格鍵再次開啟它。

為函式呼叫輸入開頭的 `(` 會插入結尾 `)`，然後叫出簽章說明，如先前所示︰

![函式的 IntelliSense 簽章說明](~/rtvs/media/intellisense-function-signature.png)
...
![使用者定義函式的 IntelliSense](~/rtvs/media/intellisense-same-file-functions.png)

![使用者定義函式的 IntelliSense 參數完成](~/rtvs/media/intellisense-parameter-completion.png)

IntelliSense 也適用於相同檔案與目前工作階段中的變數︰

![IntelliSense 變數完成](~/rtvs/media/intellisense-variable-completion.png)
...
![編輯器中的 R 智慧標籤](~/rtvs/media/intellisense-smart-tags.png)
