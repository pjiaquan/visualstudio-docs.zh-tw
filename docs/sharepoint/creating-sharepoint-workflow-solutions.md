...
### 使用精靈  
 ...
 建立工作流程之後，您可以在 \[屬性\] 視窗中修改其屬性。  雖然多數工作流程屬性可以直接在 \[屬性\] 視窗中變更，但部分屬性會需要您按一下省略符號按鈕 \(![ASP.NET Mobile 設計工具橢圓形](~/sharepoint/media/mwellipsis.gif "ASP.NET Mobile 設計工具橢圓形")\) 才能變更其值。  此按鈕會重新啟動 \[**SharePoint 自訂精靈**\]。  變更屬性值之後，選擇 \[**完成**\] 來確認完成。  
