...
![EditorConfig 階層](~/docs/ide/media/vside_editorconfig_hierarchy.png)
...
![文字編輯器定位字元設定](~/docs/ide/media/vside_editorconfig_tabsetting.png)

如同預期，在下一行按下 TAB 鍵會藉由新增四個額外的空白字元來縮排該行。

![使用 EditorConfig 前的程式碼](~/docs/ide/media/vside_editorconfig_before.png)

我們會將下列內容新增到專案中的新檔案，稱為 .editorconfig。 (`[*.cs]` 設定表示這項變更將只會適用於此專案中的 .cs 檔案。)

![新增 .editorconfig 檔案至專案](~/docs/ide/media/vside_editorconfig_addconfig.png)

現在，當您按下 TAB 鍵，您會得到定位點字元，而不是空白。

![TAB 新增定位字元](~/docs/ide/media/vside_editorconfig_tab.png)
