...
當您在自動程式碼 UI 測試專案中建立自動程式碼 UI測試時，有數個檔案會加入至您的方案。  在本主題中，我們將使用範例自動程式碼 UI 測試來探索這些檔案。  
...
###  <a name="UIMapDesignerFile"></a> UIMap.Designer.cs  
 這個檔案包含建立測試時由**自動程式碼 UI 測試產生器**自動建立的程式碼。  測試每次變更時都會重新建立這個檔案，因此這不是讓您加入或修改程式碼的檔案。  
...
#### UIMap 類別  
 這裡可讓您建立自訂程式碼來擴充 <xref:Microsoft.VisualStudio.TestTools.UITest.Common.UIMap.UIMap> 類別的功能。  測試每次修改時，**自動程式碼 UI 測試產生器**不會重新產生您在此檔案中建立的程式碼。  
...
###  <a name="CodedUITestCS"></a> CodedUITest1.cs  
 這個檔案由產生**自動程式碼 UI 測試產生器**產生，但不會在測試每次修改時重新建立，因此您可以修改這個檔案中的程式碼。  此檔案的名稱是從您建立測試時所指定的名稱產生。  
...
```  
 ...
 這個方法會呼叫您錄製測試時所指定的每個 `UIMap` 方法，這已在 [UIMap 類別](#UIMapClass)小節中說明。  
...
###  <a name="UIMapuitest"></a> UIMap.uitest  
 這是一個 XML 檔案，代表自動程式碼 UI 測試錄製及其所有部分的結構。  其中包括動作和類別，還有這些類別的方法和屬性。  [UIMap.Designer.cs](#UIMapDesignerFile) 檔案包含自動程式碼 UI 產生器為了重現測試結構而產生的程式碼，並提供測試架構的連接。  
