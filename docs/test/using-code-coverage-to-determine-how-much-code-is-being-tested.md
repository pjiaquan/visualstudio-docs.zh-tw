...
2.  若要查看已執行的程式碼行，請選擇 ![顯示程式碼涵蓋範圍著色圖示](~/docs/test/media/codecoverage-showcoloringicon.png "CodeCoverage-ShowColoringIcon")[顯示程式碼涵蓋範圍著色]。  
...
## <a name="merging-results-from-different-runs"></a>合併不同回合的結果  
 ...
 若要這樣做，請使用 ![[程式碼涵蓋範圍] 視窗中的 [合併] 按鈕圖示](~/docs/test/media/codecoverage-mergeicon.png "CodeCoverage-MergeIcon")[合併程式碼涵蓋範圍結果]。 您可以選擇最近的回合或匯入之結果的任何組合。 如果您要合併匯出的結果，必須先匯入結果。  
...
2.  在 [流程] 頁面上，展開 [自動化測試]、[測試來源]、[回合設定]。 將 [回合設定檔的類型] 設為 [已啟用程式碼涵蓋範圍]。  
 ...
 ![設定程式碼涵蓋範圍的組建定義](~/docs/test/media/codecoverage-plaincc.png "CodeCoverage-plainCC")  
