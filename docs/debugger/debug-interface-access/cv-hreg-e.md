...
> [!NOTE]
>  由於 `CV_HREG_e` 列舉太大而無法重複全部，這裡的縮寫版本做為參考提供。  子句會喪失斜體 notated。  完成列舉可在 cvconst.h 標頭檔。  
...
## 項目  
 CV\_REG\_\*  
 Intel 80x86 和 ix86 處理器使用的暫存器和 p 程式碼。  
 ...
 CV\_ALPHA\_\*  
 迪吉多 Alpha AXP 系列處理器使用的暫存器。  
  
 CV\_PPC\_\*  
 Motorola\/IBM PowerPC 處理器使用的暫存器。  
  
 CV\_SH3\_\*和 CV\_SH\_\*  
 日立 SH3 和日立 Sh 數列使用的暫存器處理器。  
  
 CV\_ARM\_\*  
 進階 RISC 機器 \(ARM\) 處理器使用的暫存器。  
  
 CV\_IA64\_\*  
 Intel IA64 系列處理器使用的暫存器。  
  
 CV\_TRI\_\*  
 英文的會 TriCore 處理器使用的暫存器。  
  
 CV\_AM33\_\*  
 松下使用的暫存器\/音樂播放牌 AM33 和相關的處理器。  
