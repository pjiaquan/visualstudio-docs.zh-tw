...
4.  在 \[名稱\]文字方塊中，輸入 `ExpenseIt`，然後選擇 \[確定\] 按鈕。  
...
4.  在 \[屬性\] 視窗中，展開 \[通用\] 分類節點並選擇 \[標題\] 屬性，然後輸入 `ExpenseIt` 並按下 **Enter** 鍵。  
...
2.  在 \[加入新項目\] 對話方塊中，選擇 \[名稱\] 文字方塊並輸入 `ExpenseItHome`，然後選擇 \[加入\] 按鈕。  
...
6.  找到行 `Title="ExpenseIt" Height="375" Width="500">` 項目並加入 `Source="ExpenseItHome.xaml"` 屬性。  
    ...
    ```  
     ...
     如同之前設定那些屬性一樣，您也可以在 \[屬性\] 視窗的 \[其他\] 分類中設定 `Source` 屬性。  
  
#### 加入詳細資料視窗  
  
1.  在**方案總管**中，開啟 **ExpenseIt** 節點的捷徑功能表，然後依序選擇 \[加入\] 和 \[頁面\]。  
  
2.  在 \[加入新項目\] 對話方塊中，選擇 \[名稱\] 文字方塊並輸入 `ExpenseReportPage`，然後選擇 \[加入\] 按鈕。  
...
2.  在 \[屬性\] 視窗中，展開 \[配置\] 分類節點，將 \[邊界\] 值設為 `10`、`10`、`0` 和 `10`，對應左、右、上、下邊界。  
...
4.  將現有的 `<ListBox>` 項目更換成下列程式碼。  
    ...
    ```  
     ...
     這個程式碼會將 `ListBox` 的 `ItemsSource` 屬性繫結 到資料來源，並套用資料範本做為 `ItemTemplate`。  
...
2.  以下列內容取代 \[名稱\] 和 \[部門\] `<StackPanel` 項目的 XAML 程式碼：  
...
## 請參閱  
 [逐步解說：建立連接至 Azure 行動服務的 WPF 桌面應用程式](../Topic/Walkthrough:%20Create%20a%20WPF%20Desktop%20Application%20connected%20to%20an%20Azure%20Mobile%20Service.md)   
